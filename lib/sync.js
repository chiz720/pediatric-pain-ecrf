...
/** Throttled: safe to call on every wake-up without hammering the endpoint. */
export async function refreshRosterIfStale() {
  if (Date.now() - lastRosterPull < ROSTER_MIN_INTERVAL) return;
  try {
    await refreshRoster();
  } catch {
    // Offline, or the endpoint is unreachable. The cached roster still works.
  }
}
...
export async function checkClock() {
  ...
  const res = await fetch(`${config.endpointUrl}?mode=health&n=${Date.now()}`, { redirect: 'follow', cache: 'no-store' });
  const body = JSON.parse(await res.text());
